...
def test_retries_after_429_then_succeeds():
    responses.add(
        responses.GET, f"{BASE}/v1/ping", json={"error_code": "0010"}, status=429, headers={"Retry-After": "1"}
    )
...
def test_persistent_429_raises_userexception():
    for _ in range(10):
        responses.add(
            responses.GET, f"{BASE}/v1/ping", json={"error_code": "0010"}, status=429, headers={"Retry-After": "0"}
        )
...
def test_persistent_500_raises_userexception():
    for _ in range(10):
        responses.add(responses.GET, f"{BASE}/v1/ping", json={"error": "internal"}, status=500)
    client = UolClient(BASE, "e@x.cz", "t", max_retries=2, sleep=lambda s: None)
    with pytest.raises(UserException):
        client.ping_request()
...
def test_throttles_when_window_exceeded():
    ...
    slept = []
    client = UolClient(
        BASE, "e@x.cz", "t", max_per_window=2, window_seconds=10.0, sleep=lambda s: slept.append(s), clock=clock
    )
    client.ping_request()
    client.ping_request()
